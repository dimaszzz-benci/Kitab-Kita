...
(function() {
  ...
  style.textContent = `
    ...
    .btn-primer:active { transform: scale(0.95); }
  `;
...
let state = {
  kitabKode : "JHN",
  pasal     : 1,
  versi     : "tb",   // ← DIGANTI: pakai "tb" untuk Terjemahan Baru
  ayatAktif : null,
  audioObj  : null,
};
...
window.addEventListener("DOMContentLoaded", function() {
  ...
  if (header) header.style.animation = "headerMasuk 0.5s ease forwards";
...
function isiDropdownVersi() {
  // Versi yang didukung bible-api.com
  const versi = [
    { kode:"tb",  nama:"TB — Terjemahan Baru" },
    { kode:"kjv", nama:"KJV — King James Version" },
    { kode:"web", nama:"WEB — World English Bible" },
    { kode:"bbe", nama:"BBE — Bible in Basic English" },
  ];
...
async function muatAyat() {
  ...
  const kitab = DAFTAR_KITAB.find(k => k.kode === state.kitabKode);
  const judulEl = document.getElementById("judul-pasal");
  if (judulEl) judulEl.textContent = kitab.nama + " " + state.pasal;

  try {
    // Format URL bible-api.com yang benar
    ...
    if (!data.verses || data.verses.length === 0) {
      // Coba fallback ke KJV kalau TB tidak tersedia
      if (state.versi === "tb") {
        tampilToast("⚠️ TB tidak tersedia, mencoba KJV...");
        const res2  = await fetch(`https://bible-api.com/${state.kitabKode}+${state.pasal}?translation=kjv`);
        const data2 = await res2.json();
        if (data2.verses && data2.verses.length > 0) {
          renderAyat(data2.verses);
          return;
        }
      }
      kontainer.innerHTML = "<p style='text-align:center;padding:30px;color:gray'>Data tidak tersedia.</p>";
      return;
    }

    renderAyat(data.verses);
    ...
    kontainer.innerHTML = `
      <div style="text-align:center;padding:30px">
        ...
        <button class="btn btn-primer" style="margin-top:14px" onclick="muatAyat()">
          🔄 Coba Lagi
        </button>
      </div>`;
  }
...
function renderAyat(verses) {
  ...
  verses.forEach((ayat, index) => {
    ...
    div.style.animationDelay = (index * 25) + "ms";
    ...
    // Bersihkan teks dari newline berlebih
    const teks = ayat.text.trim().replace(/\n/g, " ");
    div.innerHTML = `<span class="nomor-ayat">${ayat.verse}</span>${teks}`;
...
document.addEventListener("click", function() {
  const menu = document.getElementById("menu-ayat");
  if (menu && menu.classList.contains("tampil")) {
    menu.style.opacity   = "0";
    menu.style.transform = "scale(0.92)";
...
function highlightAyatAudio() {
  if (!state.audioObj || isNaN(state.audioObj.duration)) return;
  ...
  if (ayatList[idx]) {
    ayatList[idx].classList.add("aktif-audio");
    ayatList[idx].scrollIntoView({ behavior: "smooth", block: "center" });
  }
}

// ===== NAVIGASI =====
function gantiKitab() {
  state.kitabKode = document.getElementById("pilih-kitab").value;
  state.pasal     = 1;
  isiDropdownPasal();
  muatAyat();
  stopAudio();
